...
)
{
	...
	wl_iw_t *iw;
	if(status_on==false || !ap_cfg_running) {
		status_on = true;
		status_off = true;
	} else {
		return 0;
	...
	WL_TRACE(("Enter %s \n", __FUNCTION__));
#ifdef SOFTAP
	//Wait for 10 times or until _wl_control_sysioc_thread_wl_off() finished
	while(g_onoff == G_WLAN_SET_ON) {
		if(retryCount++<10) {
			WL_TRACE(("%s Retry times:%d \n", __FUNCTION__, retryCount));
			bcm_mdelay(500);
			continue;
		} else {
			break;
...
static int set_ap_cfg(struct net_device *dev, struct ap_profile *ap)
{
	...
	if (ap_cfg_running == FALSE) {
		WL_TRACE(("%s: start kernel thread:thr_wait_for_2nd_eth_dev\n", __FUNCTION__));
